   ...
   public function storeRegisterAjax2()
   {
    ...
    public function publishServiceOne()
    {
        if (IS_GET) {
            ...
            if( session('store_id') && $list_id[0]['cate_shopid'] ){
	            ...
	            $this->display();
            }elseif(!session('store_id')){
            	$this->redirect('Home/Service/storeRegister1',3,'你好！你还有开设店铺呢，3秒后前往开店');
            }elseif(!$list_id[0]['cate_shopid']){
            	$this->redirect('Home/Service/storeRegister1',3,'你好！你的店铺分类信息还没有完善，请前往完善信息');
            }
        }
        if (IS_POST) {
            //1.把接收到的数据插入到数据库
